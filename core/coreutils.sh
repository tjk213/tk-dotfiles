...
if [[ -z $DEFAULT_PYTHON_VERSION ]] ; then
    export DEFAULT_PYTHON_VERSION_MAJOR=3
    export DEFAULT_PYTHON_VERSION_MINOR=$(python3 -c 'import sys; print(sys.version_info[1])')
    if [[ "$OSTYPE" == "darwin"* ]]; then
	export DEFAULT_PYTHON_VERSION="$DEFAULT_PYTHON_VERSION_MAJOR"
    else
	export DEFAULT_PYTHON_VERSION="$DEFAULT_PYTHON_VERSION_MAJOR.$DEFAULT_PYTHON_VERSION_MINOR"
    fi
fi
